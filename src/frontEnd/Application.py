...
class Application(QtWidgets.QMainWindow):
    ...
    def openChatbot(self):
        if not CHATBOT_AVAILABLE:
            ...
            return
        try:
            if not hasattr(self, "chatbot_standalone") or self.chatbot_standalone is None:
                from frontEnd.Chatbot import ChatbotGUI
                self.chatbot_standalone = ChatbotGUI()
                self.chatbot_standalone.setWindowTitle("eSim AI Assistant")
            g = self.geometry()
            self.chatbot_standalone.resize(500, 650)
            self.chatbot_standalone.move(g.x() + g.width() - 520, g.y() + 50)
            self.chatbot_standalone.show()
            self.chatbot_standalone.raise_()
            self.chatbot_standalone.activateWindow()
            if hasattr(self.chatbot_standalone, "input_field"):
                self.chatbot_standalone.input_field.setEnabled(True)
                self.chatbot_standalone.input_field.setFocus()
            self.chatbot_window = self.chatbot_standalone
        except Exception as e:
