...
updateKicad() {
    ...
    case "$kicad_version" in
        "6.0.11")
            ...
            version="6.0.11-0~ubuntu22.04.1"
            ;;
        "7.0.11")
            kicadppa="kicad/kicad-7.0-releases"
            version="7.0.11~ubuntu22.04.1"
            ;;
        "8.0.9")
            kicadppa="kicad/kicad-8.0-releases"
            version="8.0.9-0~ubuntu22.04.1"
            ;;
        *)
            echo "Error: Invalid KiCad version specified!"
            echo "Supported versions: 6.0.11, 7.0.11, 8.0.9"
            exit 1
            ;;
    esac
    ...
    # Remove all KiCad config directories
    rm -rf ~/.config/kicad/6.0 ~/.config/kicad/7.0 ~/.config/kicad/8.0
...
function copyKicadLibrary {
    ...
    # Check for the correct KiCad version configuration folder
    if [ -d "$HOME/.config/kicad/8.0" ]; then
    ...
    elif [ -d "$HOME/.config/kicad/6.0" ]; then
        echo "KiCad 6 configuration folder already exists."
        kicad_config_dir="$HOME/.config/kicad/6.0"
    else
        echo "No KiCad config folder found. Creating based on installed version..."
        # Detect which version was installed
        if dpkg -l | grep -q "kicad.*8\."; then
            mkdir -p "$HOME/.config/kicad/8.0"
            kicad_config_dir="$HOME/.config/kicad/8.0"
        elif dpkg -l | grep -q "kicad.*7\."; then
            mkdir -p "$HOME/.config/kicad/7.0"
            kicad_config_dir="$HOME/.config/kicad/7.0"
        elif dpkg -l | grep -q "kicad.*6\."; then
            mkdir -p "$HOME/.config/kicad/6.0"
            kicad_config_dir="$HOME/.config/kicad/6.0"
        else
            # Default to 6.0 if can't detect
            mkdir -p "$HOME/.config/kicad/6.0"
            kicad_config_dir="$HOME/.config/kicad/6.0"
        fi
    fi
