...
#                            OR
#                ./install-eSim.sh --install-analog
#                            OR
#                ./install-eSim.sh --install-digital
#                            OR
...
#     ./install-nghdl.sh --install --digital       # Install NGHDL
...
function prepareNghdl
{
    echo "Preparing NGHDL..........................."
    unzip -o nghdl.zip
    cd nghdl/
    chmod +x install-nghdl.sh
}

function finishNghdl
{
    trap error_exit ERR
    ngspiceFlag=1
    cd ../
...
function installNghdlAnalog
{
    echo "Installing Analog tools through NGHDL installer..."
    prepareNghdl

    trap "" ERR
    ./install-nghdl.sh --install --analog

    finishNghdl
}

function installNghdlDigital
{
    echo "Installing Digital tools through NGHDL installer..."
    prepareNghdl

    trap "" ERR
    ./install-nghdl.sh --install --digital
...
if [ "$option" != "--install" ] && [ "$option" != "--install-analog" ] && [ "$option" != "--install-digital" ] && [ "$option" != "--uninstall" ]; then
    echo "Please select the proper operation."
    echo "--install"
    echo "--install-analog"
    echo "--install-digital"
    echo "--uninstall"
    exit 1
fi
...
if [ "$option" == "--install" ] || [ "$option" == "--install-analog" ] || [ "$option" == "--install-digital" ]; then
    ...
    if [ "$option" == "--install-analog" ]; then
        installNghdlAnalog
    elif [ "$option" == "--install-digital" ]; then
        installNghdlDigital
    else
        installNghdl
    fi
