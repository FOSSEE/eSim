import os
import json
import subprocess
import datetime
from PyQt5 import QtWidgets
from PyQt5.QtCore import QThread, pyqtSignal
from PyQt5.QtWidgets import QMessageBox, QLabel, QVBoxLayout, QPushButton, QComboBox, QProgressBar

# Define BASE_DIR as the directory where this Python file is located
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
JSON_FILE_PATH = os.path.join(BASE_DIR, "install_details.json")  # JSON file path
NGSPICE_PACKAGE_NAME = "ngspice"

# Load JSON data
def load_json_data():
    if os.path.exists(JSON_FILE_PATH):
        try:
            with open(JSON_FILE_PATH, "r") as file:
                return json.load(file)
        except (json.JSONDecodeError, FileNotFoundError):
            return {"important_packages": []}
    return {"important_packages": []}

# Save JSON data
def save_json_data(data):
    with open(JSON_FILE_PATH, "w") as file:
        json.dump(data, file, indent=4)

# Find package in JSON data
def find_package(data, package_name):
    return next((pkg for pkg in data["important_packages"] if pkg["package_name"] == package_name), None)

# Check if the package exists in the system
def is_package_installed():
    try:
        result = subprocess.run(["which", "ngspice"], capture_output=True, text=True)
        return result.returncode == 0
    except Exception as e:
        print(f"Error checking package installation: {e}")
        return False

# Update JSON if package is not installed
def update_json_if_package_missing():
    data = load_json_data()
    package = find_package(data, NGSPICE_PACKAGE_NAME)
    if not is_package_installed():
        if package:
            package.update({
                "installed": "No",
                "installed_date": None,
                "install_directory": None
            })
        else:
            data["important_packages"].append({
                "package_name": NGSPICE_PACKAGE_NAME,
                "installed": "No",
                "installed_date": None,
                "install_directory": None
            })
        save_json_data(data)

# Install Ngspice in a thread-safe way
class InstallThread(QThread):
    finished = pyqtSignal(str)

    def __init__(self, version):
        super().__init__()
        self.version = version

    def run(self):
        result = install_ngspice(self.version)
        self.finished.emit(result)

# Install Ngspice function
def install_ngspice(selected_version=None):
    try:
        print("Installing Ngspice...")

        # Step 1: Install dependencies
        subprocess.run(
            ["sudo", "apt", "install", "-y", "build-essential", "libx11-dev", "xorg-dev", "libxaw7-dev", "libreadline-dev", "wget"],
            check=True
        )

        # Step 2: Download Ngspice source
        download_url = (
            f"https://sourceforge.net/projects/ngspice/files/ng-spice-rework/old-releases/{selected_version}/ngspice-{selected_version}.tar.gz/download"
            if selected_version else
            "https://sourceforge.net/projects/ngspice/files/ng-spice-rework/38/ngspice-38.tar.gz/download"
        )
        tarball_name = os.path.join(BASE_DIR, "ngspice.tar.gz")
        subprocess.run(["wget", "-O", tarball_name, download_url], check=True)
        subprocess.run(["tar", "-xvf", tarball_name], check=True)

        # Step 3: Build and install Ngspice
        extracted_dir = next((d for d in os.listdir('.') if os.path.isdir(d) and d.startswith("ngspice")), None)
        if not extracted_dir:
            return "Error: Extracted Ngspice source directory not found."
        os.chdir(extracted_dir)
        subprocess.run(["./configure"], check=True)
        subprocess.run(["make"], check=True)
        subprocess.run(["sudo", "make", "install"], check=True)

        # Step 4: Clean up
        os.chdir(BASE_DIR)  # Return to the base directory
        if os.path.exists(tarball_name):
            os.remove(tarball_name)  # Delete the tarball
        extracted_ngspice_dir = os.path.join(BASE_DIR, extracted_dir)
        if os.path.exists(extracted_ngspice_dir):
            subprocess.run(["sudo", "rm", "-rf", extracted_ngspice_dir])  # Delete the extracted directory

        # Step 5: Update JSON details
        data = load_json_data()
        package = find_package(data, NGSPICE_PACKAGE_NAME)
        installed_date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if package:
            # Update existing entry
            package.update({
                "version": selected_version or "Latest",
                "installed": "Yes",
                "installed_date": installed_date,
                "install_directory": "/usr/local/bin"
            })
        else:
            # Create a new entry
            data["important_packages"].append({
                "package_name": NGSPICE_PACKAGE_NAME,
                "version": selected_version or "Latest",
                "installed": "Yes",
                "installed_date": installed_date,
                "install_directory": "/usr/local/bin"
            })
        save_json_data(data)  # Save the updated JSON

        return f"Ngspice version {selected_version or 'Latest'} installed successfully."
    except subprocess.CalledProcessError as e:
        return f"Installation failed: {e}"
    except Exception as e:
        return f"An error occurred: {e}"

# GUI Application
class NgspiceInstallerApp(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        update_json_if_package_missing()  # Check and update JSON file on startup
        self.init_ui()

    def init_ui(self):
        self.setWindowTitle("Ngspice Installer")
        self.setGeometry(100, 100, 400, 300)
        layout = QVBoxLayout()

        # Title Label
        title_label = QLabel("Ngspice Installer & Manager")
        title_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        layout.addWidget(title_label)

        # Check installation status
        self.refresh_package_status()

        # Installed Version Label
        self.installed_version_label = QLabel(f"Installed Version: {self.installed_version}")
        layout.addWidget(self.installed_version_label)

        # Latest Version Label
        self.latest_versions = ["38", "37", "36"]  # Update dynamically if needed
        latest_version_text = self.latest_versions[0] if self.latest_versions else "Unknown"
        self.latest_version_label = QLabel(f"Latest Version: {latest_version_text}")
        layout.addWidget(self.latest_version_label)

        # Status Label for Update Check
        self.status_label = QLabel("")
        self.update_status_message()  # Set the initial status message
        layout.addWidget(self.status_label)

        # Dropdown for Version Selection
        dropdown_label = QLabel("Select Version to Install:")
        layout.addWidget(dropdown_label)

        self.version_dropdown = QComboBox()
        self.version_dropdown.addItems(self.latest_versions)
        layout.addWidget(self.version_dropdown)

        # Progress Bar
        self.progress_bar = QProgressBar()
        self.progress_bar.setRange(0, 0)  # Indeterminate progress
        self.progress_bar.hide()
        layout.addWidget(self.progress_bar)

        # Install Button
        install_button = QPushButton("Install Ngspice")
        install_button.clicked.connect(self.install_ngspice_action)
        layout.addWidget(install_button)

        # Update Button
        update_button = QPushButton("Update Ngspice")
        update_button.clicked.connect(self.update_ngspice_action)
        layout.addWidget(update_button)

        self.setLayout(layout)

    def refresh_package_status(self):
        """Check if ngspice is installed and update the package status."""
        data = load_json_data()
        package = find_package(data, NGSPICE_PACKAGE_NAME)
        if package and package.get("installed") == "Yes":
            self.installed_version = package.get("version", "Unknown")
        else:
            self.installed_version = "The package is not installed"

    def update_status_message(self):
        """Set the status message for updates."""
        if self.installed_version == "The package is not installed":
            self.status_label.setText("The package is not installed.")
            self.status_label.setStyleSheet("color: red;")
        elif self.installed_version == self.latest_versions[0]:
            self.status_label.setText("You are using the latest version.")
            self.status_label.setStyleSheet("color: green;")
        else:
            self.status_label.setText("Your package is out of date. Please update.")
            self.status_label.setStyleSheet("color: orange;")

    def install_ngspice_action(self):
        """Install the selected version of ngspice."""
        if self.installed_version != "The package is not installed":
            QMessageBox.information(self, "Installation Status", "You have installed the package.")
            return

        selected_version = self.version_dropdown.currentText()
        self.progress_bar.show()
        self.thread = InstallThread(selected_version)
        self.thread.finished.connect(self.show_message)
        self.thread.start()

    def update_ngspice_action(self):
        """Update ngspice to the latest version."""
        latest_version = self.latest_versions[0]

        if self.installed_version == "The package is not installed":
            QMessageBox.warning(self, "Update Status", "The package is not installed. Please install it first.")
            return

        if self.installed_version == latest_version:
            QMessageBox.information(self, "Update Status", "You are already using the latest version.")
            return

        self.progress_bar.show()
        self.thread = InstallThread(latest_version)
        self.thread.finished.connect(self.show_message)
        self.thread.start()

    def show_message(self, result):
        """Display the result of the installation or update."""
        QMessageBox.information(self, "Status", result)
        self.progress_bar.hide()
        self.refresh_ui()

    def refresh_ui(self):
        """Refresh the UI to show updated versions."""
        self.refresh_package_status()
        self.installed_version_label.setText(f"Installed Version: {self.installed_version}")
        self.update_status_message()

# Main Application Loop
def main():
    update_json_if_package_missing()  # Check and update JSON file before starting the app
    app = QtWidgets.QApplication([])
    window = NgspiceInstallerApp()
    window.show()
    app.exec_()


if __name__ == "__main__":
    main()
