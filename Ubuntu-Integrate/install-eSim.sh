...
# Validate argument
if [[ $# -ne 1 ]]; then
    echo "Usage: $0 --install | --install-analog | --install-digital | --uninstall"
    exit 1
fi

ARGUMENT=$1

if [[ "$ARGUMENT" != "--install" && "$ARGUMENT" != "--install-analog" && "$ARGUMENT" != "--install-digital" && "$ARGUMENT" != "--uninstall" ]]; then
