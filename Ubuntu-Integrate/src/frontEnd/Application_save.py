...
class Application(QtWidgets.QMainWindow):
	...
	# Tool Manager - Ubuntu 22.04 Enhancement
        self.toolmanager = QtWidgets.QAction(
            QtGui.QIcon(init_path + 'images/tool-manager.png'),
        ...
        self.lefttoolbar.addAction(self.toolmanager)
    ...
    def open_toolmanager(self):
       	"""
        This function opens the Tool Manager for managing eSim packages.
        Tool Manager allows installation and updating of:
        - KiCad, Ngspice, GHDL, Verilator
        Enhanced for Ubuntu 22.04 LTS.
       	"""
        print("Function : Tool Manager")
        ...
        import os
	        ...
	        # Get the eSim home directory
        esim_home = self.obj_appconfig.eSim_HOME
	        # Path to Tool Manager
        tool_manager_path = os.path.join(esim_home, 'main.py')

        if os.path.exists(tool_manager_path):
            try:
	                # Launch Tool Manager
                subprocess.Popen(['python3', tool_manager_path])
                self.obj_appconfig.print_info('Tool Manager opened successfully')

            except Exception as e:
                self.msg = QtWidgets.QErrorMessage()
                self.msg.setModal(True)
                self.msg.setWindowTitle('Tool Manager Error')
                self.msg.showMessage(f'Error opening Tool Manager: {str(e)}')
                self.msg.exec_()
        else:
            self.msg = QtWidgets.QErrorMessage()
            self.msg.setModal(True)
            self.msg.setWindowTitle('Tool Manager Not Found')
            self.msg.showMessage(
                f'Tool Manager not found at:\n{tool_manager_path}\n\n'
                'Please ensure main.py exists in eSim directory.'
            )
            self.msg.exec_()
