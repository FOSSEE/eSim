...
class Validation:
    ...
    # Replaced validateTool method with a new one by THSS
    ...
    def validateTool(self, toolName):
        ...
        if shutil.which(toolName) is not None:
            return True
        # macOS specific paths for tools not in PATH
        if sys.platform == 'darwin':
            macos_paths = {
                ...
                'nghdl':   '/usr/local/bin/nghdl',
                'ghdl':    '/usr/local/bin/ghdl',
                'verilator': '/usr/local/bin/verilator',
            }
            if toolName in macos_paths:
                return os.path.exists(macos_paths[toolName])
        return False
