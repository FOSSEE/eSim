...
class AutoSchematic:
    def init(self, modelname, modelpath):
        ...
        if os.name == 'nt':
            ...
            esim_home = self.App_obj.parser_esim.get('eSim', 'eSim_HOME')
            self.kicad_ngveri_sym = os.path.join(
                esim_home, 'library', 'kicadLibrary',
                'eSim-symbols', 'eSim_Ngveri.kicad_sym'
            )
        else:
            self.kicad_ngveri_sym = \
